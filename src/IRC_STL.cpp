...
int IRC::start(const char *server, int port, const char *nick, const char *user, const char *name, const char *pass)
{
    ...
    {
        #ifdef WIN32
        std::string str = boost::format(
                    "Failed to connect: %d",
                    WSAGetLastError()).str();
        insert_to_buffer(str);
        #endif
        closesocket(irc_socket);
        return 1;
    }
...
int IRC_STL::quit(const std::string &quit_message) {
    if (connected)
    {
        if (quit_message.size()) {
            dataout << (boost::format("QUIT %s\r\n") % quit_message).str();
        }
        else {
            dataout << "QUIT\r\n";
        }
...
int IRC::message_loop()
{
    ...
    if (!connected)
    {
        insert_to_buffer("Not connected!");
        return 1;
    }
...
int IRC::message_loop(int &flag)
{
    ...
    const struct timeval sock_timeout={SOCK_TIMEOUT_S,SOCK_TIMEOUT_MS};
    setsockopt(irc_socket,SOL_SOCKET,SO_RCVTIMEO,(char*)&sock_timeout,sizeof(sock_timeout));
...
int IRC_STL::is_op(const std::string &channel, const std::string &nick) {
    for(auto &i : chan_users) {
        if (i.channel==channel && i.nick==nick) {
            return i.flags&IRC_USER_OP;
        }
    }
    return 0;
}

int IRC_STL::is_voice(const std::string &channel, const std::string &nick) {
    for(auto &i : chan_users) {
        if (i.channel==channel && i.nick==nick) {
            return i.flags&IRC_USER_VOICE;
        }
    }
    return 0;
}
...
void IRC::parse_irc_reply(char *data)
{
    ...
    char *chan_temp;
    hostd_tmp.target=0;
    //insert_to_buffer((boost::format("%s") % data).str());
    ...
    if (data[0]==':')
    {
        ...
        *cmd='\0';
        ...
        else if (!strcmp(cmd, "NOTICE"))
        {
            ...
            params++;
            #ifdef __IRC_DEBUG__
            insert_to_buffer((boost::format("%s >-%s- %s")
                              % hostd_tmp.nick
                              % hostd_tmp.target
                              % &params[1]).str());
            #endif
        ...
        else if (!strcmp(cmd, "PRIVMSG"))
        {
            ...
            *(params++)='\0';
            #ifdef __IRC_DEBUG__
            insert_to_buffer((boost::format("%s: %s")
                             % hostd_tmp.nick
                             % &params[1]).str());
            #endif
        }
    ...
    else
    {
        ...
        *data='\0';
        ...
        if (!strcmp(cmd, "PING"))
        {
            ...
            #ifdef __IRC_DEBUG__
            insert_to_buffer("Ping received, pong sent.");
            #endif
            fflush(dataout);
        }
        else
...
int IRC::notice(const char * fmt, ...)
{
    ...
    vfprintf(dataout, va_arg(argp, const char *), argp);
    va_end(argp);
    fprintf(dataout, "\r\n");
    return fflush(dataout);
}
...
int IRC_STL::notice(const std::string & target, const std::string &message) {
    if (!connected) {
        return 1;
    }
    dataout << (boost::format("NOTICE %s : %s\r\n")
                % target
                % message).str();
    //return fflush(dataout);
    return 0;
...
int IRC_STL::join(const std::string &channel) {
    if (!connected) {
        return 1;
    }
    dataout << (boost::format("JOIN %s\r\n")
                % channel).str();
    //return fflush(dataout);
    return 0;
}

int IRC_STL::part(const std::string & channel) {
    if (!connected) {
        return 1;
    }
    dataout << (boost::format("PART %s\r\n")
                % channel).str();
    //return fflush(dataout);
    return 0;
}

int IRC_STL::kick(const std::string & channel, const std::string & nick) {
    if (!connected) {
        return 1;
    }
    dataout << (boost::format("KICK %s %s\r\n")
               % channel
               % nick).str();
    //return fflush(dataout);
    return 0;
...
int IRC_STL::raw(const std::string & data) {
    if (!connected) {
        return 1;
    }
    dataout << (boost::format("%s\r\n")
               % data).str();
    //return fflush(dataout);
    return 0;
}
...
int IRC_STL::mode(const std::string & modes)
{
    if (!connected) {
        return 1;
    }
...
int IRC_STL::nick(const std::string & newnick)
{
    if (!connected) {
        return 1;
    }
    dataout << (boost::format("NICK %s\r\n")
                % newnick).str();
    //return fflush(dataout);
    return 0;
}

const std::string & IRC_STL::current_nick()
{
    return cur_nick;
}

void IRC_STL::insert_to_buffer(const std::string &str) {
    _obuf.push_back(str);
}

void IRC_STL::clear_buffer() {
    _obuf.clear();
}

const std::deque<std::string>& IRC_STL::output_buffer() {
    return _obuf;
}
